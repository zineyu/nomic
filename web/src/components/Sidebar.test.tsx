...
//
// 按 project 分组（可折叠）的 work 列表（ADR-0044：work 为一等入口，点击行
// 恢复其主 session），项目组标题为卡片样式。
// 组标题右侧带「新建会话」按钮（在该 project 下创建 work）；「项目」标题行带
// 「添加项目」内联输入。每个 work 只展示标题，长标题被 CSS 截断但可通过
// title 读取全文；当前 work（主 session）以主色高亮；侧栏与聊天区之间有右侧
// 分隔边框。无默认 project：新建必须经组标题按钮显式指定归属。
...
// work 夹具：main_session_id 与 id 同值（点击行 = 恢复主 session）
const works = [
  { id: 'a', main_session_id: 'a', title: LONG_TITLE, project_id: 'wa', project: '/tmp', session_count: 1, first_message_at: null, last_message_at: 1786960000000, message_count: 101 },
  { id: 'b', main_session_id: 'b', title: null, project_id: 'wb', project: '/tmp', session_count: 1, first_message_at: null, last_message_at: 1786960000000, message_count: 3 },
]

const groupedWorks = [
  { id: 'a1', main_session_id: 'a1', title: '项目 A 会话一', project_id: 'wa', project: '/home/zine/alpha', session_count: 1, first_message_at: null, last_message_at: 300, message_count: 5 },
  { id: 'b1', main_session_id: 'b1', title: '项目 B 会话一', project_id: 'wb', project: '/home/zine/beta', session_count: 1, first_message_at: null, last_message_at: 200, message_count: 2 },
  { id: 'a2', main_session_id: 'a2', title: '项目 A 会话二', project_id: 'wa', project: '/home/zine/alpha', session_count: 1, first_message_at: null, last_message_at: 100, message_count: 1 },
]
...
describe('Sidebar', () => {
  ...
  it('会话按 project 分组，组标题为路径最后一段且 title 为完整路径', () => {
    renderSidebar({ works: groupedWorks })
  ...
  it('组标题可折叠/展开，默认全部展开，且各组折叠互不影响', async () => {
    ...
    // 折叠 alpha 组：其 work 消失，beta 组不受影响
  ...
  it('组标题「新建会话」按钮在该 project 下创建会话', async () => {
    ...
    renderSidebar({ works: groupedWorks, onNewSession })
    const alphaGroup = screen.getByRole('region', { name: '/home/zine/alpha' })
    await user.click(
  ...
  it('重命名会话：行内按钮展开内联编辑，Enter 提交并关闭', async () => {
    const user = userEvent.setup()
    const onRenameWork = vi.fn().mockResolvedValue(undefined)
    renderSidebar({ onRenameWork })
    ...
    expect(onRenameWork).toHaveBeenCalledWith('a', '改名后的会话')
  ...
  it('重命名会话：Esc 取消不提交', async () => {
    ...
    expect(onRenameWork).not.toHaveBeenCalled()
    expect(screen.queryByRole('textbox', { name: '会话标题' })).not.toBeInTheDocument()
  })

  it('重命名会话失败时就地展示错误，输入框保留', async () => {
    const user = userEvent.setup()
    const onRenameWork = vi.fn().mockRejectedValue(new Error('session not found'))
    renderSidebar({ onRenameWork })
  ...
  it('删除会话：确认对话框说明不可恢复，确认后调用删除', async () => {
    const user = userEvent.setup()
    const onDeleteWork = vi.fn().mockResolvedValue(undefined)
    renderSidebar({ onDeleteWork })
    ...
    expect(onDeleteWork).toHaveBeenCalledWith('a')
    // 成功后对话框关闭
  ...
  it('删除会话失败时错误展示在对话框内，对话框保留', async () => {
    const user = userEvent.setup()
    const onDeleteWork = vi.fn().mockRejectedValue(new Error('session 库不可用'))
    renderSidebar({ onDeleteWork })
