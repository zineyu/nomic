...
- run 结束判定收在 `GoalNudger`（`tui/goal.rs`）：目标已完成 → 换回正常
  工具集并通知；run 正常结束而未完成 → 以复述目标的提示词追问（计数
  +1）；连续追问达上限（3 次）暂停，目标仍进行中，用户手动继续或取消。
  追问与队列的优先级沿用 ADR-0014 的口径：队列非空时队列优先。
