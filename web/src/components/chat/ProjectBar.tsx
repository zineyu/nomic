// 启动页项目选择栏：在输入框上方选择新会话的归属项目（无默认 project，
// 必须显式选择）。已登记 project 以下拉选择；「使用其他目录…」切换为内联
...
export function ProjectBar({ projects, value, onChange }: ProjectBarProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-page px-4 pt-2 sm:px-7">
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              {projects.map((ws) => (
                ...
                >
                  <span className="flex min-w-0 flex-col">
                    <span className="truncate">{projectName(ws.path)}</span>
        ...
        {!editing && value && !projects.some((ws) => ws.path === value) && (
