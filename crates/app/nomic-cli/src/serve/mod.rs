//! serve 模式（`--serve`）：headless HTTP 服务（axum）+ 纯 WebSocket 事件流。
//!
//! 架构见 `docs/adr/0030-web-ui.md`（服务端协议）与 `docs/adr/0046-flutter-gui.md`
//! （Web UI 前端移除、由 Flutter GUI 替代）。复用 `bootstrap` 运行时与
//! print/TUI 的事件落库 seam（[`SessionRecorder`] 一行接线）；agent actor
//! （ADR-0022）经 [`nomic_core::AgentHandle`] 驱动。
//!
//! GUI ↔后端通信全部通过 `ws://{host}/ws` 双向事件流：
...
pub struct SessionRuntime {
    ...
    /// steering 统一消息队列（ADR-0014/0027，serve 侧实现）：运行中提交的
...
pub async fn run(cli: &Cli) -> Result<()> {
    ...
    println!("\x1b[36m▸ nomic serve: ws://{local}/ws\x1b[0m");
    println!(
        "\x1b[2m  cwd: {} · 事件流服务（Flutter GUI 连接此地址）\x1b[0m",
        std::env::current_dir().map_or_else(|_| "?".into(), |p| p.display().to_string()),
    );
    tracing::info!(%local, "nomic serve started");
...
/// ECHO 避免回显。OPOST/ONLCR、ISIG 等其余终端标志保持原样：serve 模式
