...
### 2026-08-03：branch 切换落地，相对计数在分支路径上依然精确

「已知限制」预期的 branch 切换已由 `/tree` 命令落地（TUI 选择非工具调用
条目作为新分支起点，见 ADR-0001 Amendments）。落地时发现无需改回绝对指针：
`load_branch` 沿所选 entry 的**祖先路径**重放，路径前缀即压缩发生时 agent
实际持有的上下文，`kept_count` 相对该前缀截尾，在任意分支路径上逐字节精确。
原限制担忧的「跨分支路径重放」场景不存在——祖先路径唯一，压缩条目永远在其
发生时的路径上。仅当未来支持**跨分支移动/复用 entry**（脱离祖先路径的
重放）时，才需要 `first_kept_entry_id` 绝对指针。
