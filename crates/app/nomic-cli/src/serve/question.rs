//! `ask_user_question` 工具的 serve 实现：问题经 WebSocket 推给前端弹层，回答经
...
//! [`SessionRuntime`][crate::serve::SessionRuntime]），避免跨 session 串流。
...
/// serve 模式的问题宿：问题广播到本 session 的 WebSocket，等待前端回答。
#[derive(Debug)]
