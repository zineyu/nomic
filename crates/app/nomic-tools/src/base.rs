//! 工具基准目录（project 严格归属）：session 内工具的相对路径以其
//! project 路径解析；未设置时退回进程 cwd（由 OS 隐式解析，行为同现状）。
//!
//! 句柄本体在 [`nomic_vfs::ProjectRoot`]（`local://` VFS 与各
//! 文件工具共享同一份状态）；本模块保留 `BaseDir` 别名与路径解析助手。

use std::path::{Path, PathBuf};

/// 工具共享的基准目录句柄（[`nomic_vfs::ProjectRoot`] 的兼容别名）。
pub type BaseDir = nomic_vfs::ProjectRoot;
