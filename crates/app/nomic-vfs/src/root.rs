//! 会话 project 根句柄：内部 URI 协议（`local://`）与文件工具共用的
//! 基准目录。clone 即共享同一份状态；交互端切换 session project 时经
//! [`ProjectRoot::set`] 原地更新，下一次工具执行即用新基准。
...
pub struct ProjectRoot(Arc<RwLock<Option<PathBuf>>>);
...
impl ProjectRoot {
    ...
    /// 更新根目录（切换到另一个 project）。
    pub fn set(&self, dir: PathBuf) {
        tracing::debug!(dir = %dir.display(), "project root updated");
        *self
            .0
            .write()
...
impl From<Option<PathBuf>> for ProjectRoot {
