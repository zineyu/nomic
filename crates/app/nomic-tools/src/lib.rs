...
pub fn default_tools_with_skills_in_shared(
    ...
    // 会话级 VFS 挂载表（ADR-0042/0043）：工具共享同一实例，装配与
    // 系统提示词目录渲染共用 fs::session_router，保证两侧挂载集一致。
    // 后续协议（artifact/history…）在 session_router 中追加挂载。
    let vfs_router =
        std::sync::Arc::new(nomic_vfs::fs::session_router(skill_resolver, base.clone()));
    let nix_env = nix_env::NixEnvCache::new();
    prewarm_nix_env(&nix_env, base);
    vec![
        nomic_core::DynTool::new(
